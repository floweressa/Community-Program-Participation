...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($password)) {
        ...
        // Allow login by email or by a staff ID pattern (EMP-12345 → stored as email)
        $stmt = $db->prepare(
...
<head>
    ...
    <title>Bayanihan - Staff Login</title>
</head>
<style>
...
<div class="container">
    <section class="left-content">
        ...
        <h1>Manage your <br><span>community</span></h1>
        <p>Access the administrative dashboard to manage local programs, verify participants, and generate community impact reports.</p>
    </section>
        ...
        <div class="login-card">
            ...
            <form method="POST" action="staff_login.php">
                <div class="input-group">
                    ...
                    <input type="text" name="username" placeholder="staff@bayanihan.org"
                           value="<?= htmlspecialchars($_POST['username'] ?? '') ?>" required>
                </div>
                <div class="input-group">
                    <label>Password</label>
                    <input type="password" name="password" placeholder="••••••••" required>
                    <a href="#" class="forgot-link">Reset Staff Password?</a>
                </div>
